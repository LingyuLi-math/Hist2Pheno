...
# python -u code/Xenium_lung/Lung_train_validate_cv_UNIlabel.py \
#   --mode cross-dataset --sample THD0008 \
#   --pooled-save-result result_all_spatial \
#   --use-spatial-context --spatial-k 8 --spatial-mode mean \
#   --ablation-tag D_emph_L2_spatial_bs4096 \
#   --steps stardist_all
...
########################################################
# 2026.07.03 LLY: add other four levels predictions
########################################################
...
DEFAULT_DATA_ROOT = (
    _REPO_ROOT
    / "data/Xemium/weiqin/SpatialPF-NGenetics/Spatial-PF-Processed/Data"
)
...
POOLED_STARDIST_LEVELS: tuple[str, ...] = ("l2", "l1", "l12", "l3", "l4")
    ...
    *,
    ...
    if extra_tiers:
        if not hasattr(model_star, "level12_head"):
            print("  Skip L12/L3/L4 StarDist: not five-head.", flush=True)
        else:
            head_probs = mlp_collect_five_head_softmax_probs(
                model_star, x_star_scaled[:m], ctx.device
            )
            star_heads = stardist_head_preds(
                model_star, scaler, star["X_star"], ctx.device, predict_all_label_heads
            )
            plot_stardist_spatial_extra(
                plot_tier_spatial_distribution,
                plot_celltype_spatial_distribution,
