## 2026.09.03 LLY: CODEX GBM CLI twin of CODEX_hcc / Xenium_lung UNI-label train
## Paths: data/CODEX/GBM/Cases/{P174511_Initial,P179161_Recurrent}/project_all_UNI/
## Samples: P174511_Initial and P179161_Recurrent (10x FFPE Human Breast Cancer preview)
...
## Cross-dataset pool of Ini+Rec + spatial kNN (recommended):
# python -u code/CODEX_gbm/GBM_train_validate_cv_UNIlabel.py \
#   --mode cross-dataset \
...
# python -u code/CODEX_gbm/transer_embedding_label_h5ad.py --sample P174511_Initial --steps stardist_all_h5ad


#!/usr/bin/env python3
"""
CODEX GBM: stratified K-fold HCE training + HE internal validation + StarDist external validation.

Mirrors ``GBM_train_validate_cv_UNIlabel_single.ipynb`` and the HCC three-head CLI.
UNI embeddings live under ``Cases/{sample}/project_all_UNI/ImgEmbeddings_all/``.
GT / StarDist cell CSVs come from ``match_codex_cells_with_pixel.py``; StarDist
centroids from ``data/CODEX/GBM/WangLab/StarDist_Segment/``.

By default processes **P174511_Initial and P179161_Recurrent** and enables spatial neighbor fusion with
...
``data/CODEX/GBM/Results/{save_result}/`` (cross-dataset).
"""
...
_GBM_DIR = _REPO_ROOT / "code" / "CODEX_gbm"
for _p in (_PKG_DIR, _GBM_DIR):
...
DEFAULT_DATA_ROOT = DEFAULT_GBM_ROOT / "Results"
DEFAULT_STARDIST_ROOT = DEFAULT_GBM_ROOT / "WangLab" / "StarDist_Segment"

STARDIST_ALL_H5AD_SUFFIX = "_all_features_stardist.h5ad"
STARDIST_GBM_REST_RESULT_SUBDIR = "stardist_gbm_rest"
INCOMPLETE_CASES_DIRNAME = "Incomplete_Cases"


def stardist_gbm_rest_all_label_h5ad_path(data_root, sample, save_result="result"):
    """``Results/{save_result}/stardist_gbm_rest/{sample}/..._label.h5ad``."""
    result_dir = Path(data_root) / save_result / STARDIST_GBM_REST_RESULT_SUBDIR / sample
...
def load_he_acq_map(cases_root: Path | None = None) -> dict[str, str]:
    """GBM sample id is also the UNI ``sc_{sample}`` prefix."""
...
def discover_brca_samples(
    ...
    *,
    ...
    """
    Return MATCHED_HE keys for the CODEX GBM P174511_Initial/P179161_Recurrent.
    ...
    if not out:
        raise FileNotFoundError(
            f"No GBM samples under {cases_root}. "
...
def prepare_pooled_cv_data_from_h5ads(
    ...
    """Pool HE matched h5ads for GBM three-head (L2/L12/L1)."""
...
def resolve_data_root(args: argparse.Namespace, cases_root: Path) -> Path:
    """Pooled result parent (``data/CODEX/GBM/Results``). Override with --data-root."""
...
def step_prepare_cv(ctx: RunContext) -> None:
    ...
    if use_three_head:
        print(
            "  Three-head training (GBM): L2=final_CT, L12=final_sublineage, "
            "L1=final_lineage",
            flush=True,
        )
...
def step_pooled_prepare(ctx: PooledRunContext) -> None:
    ...
    if use_three_head:
        print(
            "  Three-head pooled training (GBM): L2=final_CT, L12=final_sublineage, "
            "L1=final_lineage",
            flush=True,
        )
    ctx.g.update(
...
POOLED_STARDIST_LEVELS: tuple[str, ...] = ("l2", "l1", "l12")  # GBM three-head
...
def step_pooled_stardist_all_hcc_rest(
    ...
    *,
    ...
    GBM preview has none; kept so the HCC-style all notebook can call it.
    Output: ``Results/{save_result}/stardist_gbm_rest/{sample}/..._label.h5ad``
    """
    ...
    if not samples:
        print(
            f"\n[Pooled StarDist all nuclei — GBM rest] no extra samples "
    ...
    print(
        f"\n[Pooled StarDist all nuclei — GBM rest] {len(samples)} datasets",
        flush=True,
    )
    failures = []
    for sample in samples:
        try:
            out_path = stardist_gbm_rest_all_label_h5ad_path(
                ctx.data_root, sample, ctx.save_result
            )
    ...
    if failures:
        print(
            f"\n  StarDist-all GBM rest failures: {len(failures)}/{len(samples)}",
            flush=True,
        )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="CODEX GBM UNI-label train/validate (P174511_Initial and P179161_Recurrent; HE + StarDist).",
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help=f"Cases root with P174511_Initial/P179161_Recurrent dirs (default: {DEFAULT_CASES_ROOT}).",
    )
    p.add_argument(
        "--data-root",
        type=Path,
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Single MATCHED_HE key (default: P174511_Initial and P179161_Recurrent).",
    )
    p.add_argument(
        "--therapy-model",
        type=str,
    ...
    p.add_argument(
        ...
        "cross-dataset: pool GBM h5ads, dataset-level CV, "
    ...
    p.add_argument(
        ...
        help=(
            ...
            "three_tier_auc_sum = sum of L2+L1+L12 macro AUROC (GBM default)."
        ),
    )
    p.add_argument(
        "--cv-selection-metric",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.mode == "cross-dataset":
        ...
        for sample in samples:
            if sample not in acq_map:
                raise KeyError(f"Unknown GBM sample {sample!r}")
        ...
        print(f"Mode:       cross-dataset (pooled GBM)", flush=True)
    ...
    print(f"Cases root: {cases_root}", flush=True)
    print(f"Samples:    {len(samples)} (CODEX GBM P174511_Initial/P179161_Recurrent)", flush=True)
    print(f"Steps:      {', '.join(sorted(steps))}", flush=True)
    print(f"Device:     {device}", flush=True)
    print(f"Train batch size: {args.train_batch_size}", flush=True)
    ...
    for sample in samples:
        if sample not in acq_map:
            failures.append((sample, f"Unknown GBM sample {sample!r}"))
            skipped += 1
            continue
        ctx = RunContext(
            sample=sample,
