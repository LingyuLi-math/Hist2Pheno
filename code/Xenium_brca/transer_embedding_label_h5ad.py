...
#!/usr/bin/env python3
"""
Build matched AnnData files for Xenium BRCA HE regions.
...
DEFAULT_CASES_ROOT = _REPO_ROOT / "data/Xemium/BRCA/Cases"
DEFAULT_STARDIST_ROOT = _REPO_ROOT / "data/Xemium/BRCA/StarDist_Segment"
DEFAULT_THERAPY_MODEL = "project_all_UNI"
MAPPING_CSV_NAME = None  # BRCA samples are rep1/rep2
...
## 2026.09.03 BRCA: sample id is the UNI embedding prefix (sc_rep1 / sc_rep2)
def load_acquisition_map(
    cases_root: Path, *, aligned_only: bool = True
) -> dict[str, str]:
    """rep1/rep2 map to themselves (no CODEX acquisition table)."""
...
def discover_samples(
    ...
    """Return mapped BRCA regions with directories under cases_root."""
    if requested_sample is not None:
        ...
        if requested_sample not in acq_map:
            raise KeyError(
                f"Unknown BRCA sample {requested_sample!r}; choose rep1 or rep2"
            )
    ...
    if not samples:
        raise FileNotFoundError(
            f"No mapped BRCA sample directories found under {cases_root}"
        )
    return samples
...
def _build_matched_h5ad(
    *,
    ...
    """Build a BRCA labeled h5ad with stricter cache metadata than base.py."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build Xenium BRCA matched and all-nuclei UNI h5ad files.",
    ...
    parser.add_argument(
        "--sample",
        default=None,
        help="One sample id (rep1 or rep2); default processes both on-disk BRCA regions.",
    )
    parser.add_argument(
        "--steps",
        nargs="+",
