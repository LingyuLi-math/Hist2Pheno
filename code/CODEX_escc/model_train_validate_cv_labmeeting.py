    ...
    try:
        ...
        if use_logo:
            ...
            LP = run_logo_cv_with_insample_report(
                ...
                loader_kwargs={"seed": args.seed, "train_balance_sampler": False},
        ...
        if args.run_stardist_eval:
            ...
            # Prefer this run's best checkpoint (timestamp folder) when it exists and
            # the StarDist model source equals the current training therapy_model.
            stardist_ckpt_path = None
            if (
                stardist_model_therapy_model == therapy_model
                and best_model_path.is_file()
            ):
                stardist_ckpt_path = str(best_model_path)
                print(f"Using current run checkpoint for StarDist model: {stardist_ckpt_path}")
            else:
                print(
                    "Using default checkpoint resolution for StarDist model "
                    f"(therapy_model={stardist_model_therapy_model})."
                )
            ...
            model_star = load_model_for_predict(
                ...
                stardist_model_therapy_model,
                device=device,
                checkpoint_path=stardist_ckpt_path,
            )
            ...
            stardist_spatial_kwargs = {}
            if args.parent_value_stardist == "all":
                stardist_spatial_kwargs = {
                    "spatial_figsize": (args.spatial_fig_w, args.spatial_fig_h),
                    "spatial_point_size": args.spatial_point_size,
                }
        ...
        if best_model_path.exists():
            ...
            if best_model_path.resolve() != model_copy_path.resolve():
                ...
                print("Best model already stored in current run directory.")
        else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Best checkpoint path. Default: current run timestamp folder (<result_dir>/best_mlp_gpu.pt).",
    )
...
##############################################################
# 2026.04.06   Tumor1 CV
##############################################################

## train_validate_20260406231524545201 [hidden_dims]
# python code/model_train_validate_cv.py \
#     --therapy_data NCRT \
#     --parent_value tumor1 \
#     --cuda_device 2 \
...
## train_validate_20260406233952763417 [default]
# python code/model_train_validate_cv.py \
#     --therapy_data NCRT \
#     --parent_value tumor1 \
#     --cuda_device 2 \
...
## plot all: train_validate_20260407161448025350
# python code/model_train_validate_cv.py \
#   --therapy_data NCRT \
#   --parent_value all \
#   --cuda_device 2 \
...
##############################################################
# 2026.04.07    using trained model to predict all
#               Forexample: train_validate_20260407161448025350
#               under default parameters: 1024 512 256 hidden_dims, 0.2 dropout, 1e-3 lr, 5e-5 weight_decay
##############################################################
# bash /home/lingyu/ssd2/Python/Hist2Pheno/code/CODEX_escc/demo_NICT_cv_infer.sh