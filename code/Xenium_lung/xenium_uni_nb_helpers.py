...
def plot_stardist_roc_extra(plot_multiclass_roc_curves, result_fig, head_probs, cv_data, m, g=None, tiers=None):
    g = g or {}
    tiers_set = None if tiers is None else {str(t).lower() for t in tiers}
    roc_specs = (
...
def merge_stardist_auroc_with_clinical(
    ...
    *,
    ...
    """Attach ``Status`` / ``Sample_Affect_Pairing`` via ``Donor_ID`` (= sample folder name)."""
    import pandas as pd
    ...
    for col in STARDIST_CLINICAL_COLUMNS:
...
def analyze_stardist_macro_auroc_by_clinical(
    ...
    *,
    ...
    """
    Compare sample-level StarDist macro AUROC across clinical groups (Status, Sample_Affect_Pairing).

    Uses Mann–Whitney U (2 groups) or Kruskal–Wallis (3+), with boxplots matching
    ``histology_derived_niche_index.plot_q4_clinical_comparison``.
    """
    import matplotlib.pyplot as plt
    from histology_derived_niche_index import plot_q4_clinical_comparison, test_q4_by_clinical_groups
    ...
    merged = merge_stardist_auroc_with_clinical(auroc_df, clinical_info)
    ...
    for tier in tiers:
        ...
        if metric_col not in merged.columns or not np.isfinite(merged[metric_col]).any():
            print(f"  Skip {tier_key.upper()}: no finite {metric_col}", flush=True)
            continue
        stats = test_q4_by_clinical_groups(
            merged,
            q4_col=metric_col,
            clinical_columns=clinical_columns,
            alternative=alternative,
        )
        print(f"\n=== {tier_key.upper()} macro AUROC vs clinical groups ===", flush=True)
        display_fn = None
        try:
        ...
        if save_dir is not None:
            ...
            save_path = save_dir / f"stardist_{tier_key}_macro_auroc_clinical_comparison.pdf"
    ...
    return {"auroc_df": auroc_df, "merged": merged, "by_tier": by_tier}
########################################################
