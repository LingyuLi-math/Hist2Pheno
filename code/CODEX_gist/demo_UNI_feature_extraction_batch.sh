#!/usr/bin/env bash
## CODEX GIST s1167 — batch UNI feature extraction
#
# cd /home/lingyu/ssd2/Python/Hist2Pheno
#
# Usage:
#   bash code/CODEX_gist/demo_UNI_feature_extraction_batch.sh [gt|stardist]
#
# Default: all 550 annotated GIST TMA cores.
#
# Env: COORD, ACQ_ID, STARDIST_ROOT, CLEAN_UNI_OUTPUT, SKIP_IF_DONE, PYTHON
#
# Examples:
...
#   ACQ_ID=Charvill-94_c013_v001_r001_reg002 bash code/CODEX_gist/demo_UNI_feature_extraction_batch.sh stardist

set -euo pipefail

REPO="/home/lingyu/ssd2/Python/Hist2Pheno"
SINGLE_SCRIPT="${REPO}/code/CODEX_gist/demo_GT_feature_extraction_Single.sh"
if [[ -z "${PYTHON:-}" ]]; then
  for cand in \
...
echo "=== CODEX GIST UNI feature extraction (batch) ==="
...
repo = Path("${REPO}")
sys.path.insert(0, str(repo / "code" / "CODEX_gist"))
from match_codex_cells_with_pixel import list_aligned_annotated_regions
df = list_aligned_annotated_regions()
...
TOTAL="${#ACQ_ROWS[@]}"
if (( TOTAL == 0 )); then
  echo "ERROR: no GIST cores to process" >&2
  exit 1
fi
