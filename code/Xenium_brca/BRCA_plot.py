...
#####################################################
# 2026.09.03 Hist2Pheno pooled cell-type bar charts
#####################################################
...
def brca_celltype_color_map() -> dict[str, str]:
    """Paper hex colors; accept both ``DCIS_1`` and ``DCIS 1`` spellings."""
    colors: dict[str, str] = {}
    for name, hex_color in ctype_hex_map.items():
        colors[str(name)] = str(hex_color)
        colors[str(name).replace("_", " ")] = str(hex_color)
    return colors
...
def plot_pooled_celltype_distribution(
    ...
    *,
    ...
    title: str = "Xenium BRCA pooled cell-type distribution (rep1 + rep2)",
    **kwargs,
):
    """Plot one bar chart pooling BRCA replicates. Colors from ``ctype_hex_map``."""
    ...
    if exclude_unknown:
        drop = [name for name in counts.index if str(name).strip() in {"Unknown", "Unlabeled"}]
        if drop:
            counts = counts.drop(index=drop)
    kwargs.setdefault("celltype_colors", brca_celltype_color_map())
