## 2026.08.20 LLY, copy from CODEX_hcc/transer_embedding_label_h5ad.py
## PDAC s1167: sample key is ACQUISITION_ID (no MATCHED_HE).
## Annotated (278): he_h5ad / stardist_csv / stardist_h5ad / stardist_all_h5ad
## Incomplete_Cases (195, no cell-type CSV): stardist_all_h5ad only
...
#!/usr/bin/env python3
"""
Build matched AnnData files for CODEX PDAC (s1167 Pancreas TMA) cores.

Sample key is ``ACQUISITION_ID``. The four explicit pipeline steps are:

1. ``he_h5ad``: ``*_cells_with_pixel.csv`` + HE embeddings
   -> ``*_matched_features.h5ad``.
2. ``stardist_csv``: transfer CODEX labels to raw StarDist centroids
...
``--steps all`` means steps 1-3. Step 4 remains explicit.
``--incomplete`` selects the 195 PDAC cores without cell-type CSV
(Incomplete_Cases analog). Those cores only support ``stardist_all_h5ad``.
"""
...
DEFAULT_STARDIST_ROOT = (
    _REPO_ROOT / "data/CODEX/HCC/StarDist_Segment_pdac/pdac_result"
)
...
def load_acquisition_map(
    ...
    *,
    ...
    """ACQUISITION_ID → ACQUISITION_ID for Pancreas TMA cores.

    ``incomplete=False``: 278 annotated cores.
    ``incomplete=True``: 195 Incomplete_Cases (no cell-type CSV).
    """
    del cases_root  # sample dirs are resolved relative to s1167 via metadata
    region_df = (
        list_incomplete_pdac_regions()
        if incomplete
        else list_aligned_annotated_regions()
    )
    out: dict[str, str] = {}
    for _, row in region_df.iterrows():
        acq_id = str(row["ACQUISITION_ID"]).strip()
        if acq_id and acq_id.lower() != "nan":
            out[acq_id] = acq_id
    return out
...
def discover_samples(
    ...
    *,
    ...
    """Return PDAC cores with directories under cases_root."""
    if requested_sample is not None:
        if requested_sample not in acq_map:
            acq_map = load_acquisition_map(cases_root, incomplete=False)
            acq_map.update(load_acquisition_map(cases_root, incomplete=True))
        if requested_sample not in acq_map:
            raise KeyError(
                f"Unknown PDAC ACQUISITION_ID={requested_sample!r}"
            )
    ...
    if not samples:
        kind = "Incomplete_Cases" if incomplete else "annotated PDAC"
        raise FileNotFoundError(
            f"No {kind} directories found under {cases_root}"
        )
    return samples
...
def _build_matched_h5ad(
    *,
    ...
    """Build an HCC labeled h5ad with stricter cache metadata than base.py."""
...
def build_stardist_csv(paths: SamplePaths, *, overwrite: bool) -> Path:
    """Step 2: reuse the HCC one-to-one CODEX-to-StarDist matching helper."""
    _require_file(paths.cells_csv, "CODEX cells-with-pixel CSV")
    ...
    if matched.empty:
        raise ValueError("CODEX-to-StarDist matching produced zero rows")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build CODEX PDAC matched and all-nuclei UNI h5ad files.",
    ...
    parser.add_argument(
        ...
        help="s1167 root containing {ACQUISITION_ID} directories.",
    )
    parser.add_argument(
        "--stardist-root",
        type=Path,
        default=DEFAULT_STARDIST_ROOT,
        help="StarDist_Segment_pdac/pdac_result root.",
    )
    parser.add_argument(
        "--sample",
        default=None,
        help="One ACQUISITION_ID; default is all 278 annotated PDAC cores.",
    )
    parser.add_argument(
        "--incomplete",
        action="store_true",
        help="Process 195 Incomplete_Cases (no cell-type CSV). Use with --steps stardist_all_h5ad.",
    )
    parser.add_argument(
        "--steps",
        nargs="+",
    ...
    parser.add_argument(
        "--overwrite-csv",
        action="store_true",
        help="Rebuild the CODEX-labeled StarDist CSV.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    labeled_steps = steps & {"he_h5ad", "stardist_csv", "stardist_h5ad"}
    if args.incomplete and labeled_steps:
        raise ValueError(
            "--incomplete cores have no cell-type CSV; use --steps stardist_all_h5ad only."
        )
    acq_map = load_acquisition_map(cases_root, incomplete=args.incomplete)
    samples = discover_samples(
        cases_root, args.sample, acq_map, incomplete=args.incomplete
    )
