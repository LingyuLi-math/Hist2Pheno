## 2026.09.03 LLY: Xenium BRCA CLI twin of CODEX_hcc / Xenium_lung UNI-label train
## Paths: data/Xemium/BRCA/Cases/{rep1,rep2}/project_all_UNI/
## Samples: rep1 and rep2 (10x FFPE Human Breast Cancer preview)
## Hierarchy: three-head (L2/L12/L1); selection metric three_tier_auc_sum
## Spatial: build_spatial_neighbor_index defaults match lung (k=8, mode=mean); enabled by default

## cd /home/lingyu/ssd2/Python/Hist2Pheno
## conda activate SeededNTM

## Preprocess CSVs (once per replicate):
...
# python -u code/Xenium_brca/BRCA_train_validate_cv_UNIlabel.py --sample rep1

## Both replicates (per-sample):
# python -u code/Xenium_brca/BRCA_train_validate_cv_UNIlabel.py

## Cross-dataset pool of rep1+rep2 + spatial kNN (recommended):
# python -u code/Xenium_brca/BRCA_train_validate_cv_UNIlabel.py \
#   --mode cross-dataset \
...
## All-nuclei StarDist h5ad:
# python -u code/Xenium_brca/transer_embedding_label_h5ad.py --sample rep1 --steps stardist_all_h5ad


#!/usr/bin/env python3
"""
Xenium BRCA: stratified K-fold HCE training + HE internal validation + StarDist external validation.

Mirrors ``BRCA_train_validate_cv_UNIlabel_single.ipynb`` and the HCC three-head CLI.
UNI embeddings live under ``Cases/{sample}/project_all_UNI/ImgEmbeddings_all/``.
GT / StarDist cell CSVs come from ``match_xenium_cells_with_pixel.py``; StarDist
centroids from ``data/Xemium/BRCA/StarDist_Segment/``.

By default processes **rep1 and rep2** and enables spatial neighbor fusion with
lung defaults (``build_spatial_neighbor_index``, k=8, mode=mean).
...
CSV outputs under ``Cases/{sample}/project_all_UNI/result/`` (per-sample) or
``data/Xemium/BRCA/Results/{save_result}/`` (cross-dataset).
"""
...
_BRCA_DIR = _REPO_ROOT / "code" / "Xenium_brca"
for _p in (_PKG_DIR, _BRCA_DIR):
...
DEFAULT_DATA_ROOT = DEFAULT_BRCA_ROOT / "Results"
DEFAULT_STARDIST_ROOT = DEFAULT_BRCA_ROOT / "StarDist_Segment"

STARDIST_ALL_H5AD_SUFFIX = "_all_features_stardist.h5ad"
STARDIST_BRCA_REST_RESULT_SUBDIR = "stardist_brca_rest"
INCOMPLETE_CASES_DIRNAME = "Incomplete_Cases"


def stardist_brca_rest_all_label_h5ad_path(data_root, sample, save_result="result"):
    """``Results/{save_result}/stardist_brca_rest/{sample}/..._label.h5ad``."""
    result_dir = Path(data_root) / save_result / STARDIST_BRCA_REST_RESULT_SUBDIR / sample
    return result_dir / f"{sample}{STARDIST_ALL_H5AD_SUFFIX}"
...
def load_he_acq_map(cases_root: Path | None = None) -> dict[str, str]:
    """BRCA sample id is also the UNI ``sc_{sample}`` prefix."""
    return {sid: sid for sid in list_aligned_annotated_regions()["MATCHED_HE"].tolist()}


def discover_brca_samples(
    cases_root: Path,
    ...
    *,
    ...
    """
    Return MATCHED_HE keys for the Xenium BRCA rep1/rep2.

    When ``sample`` is set, return that one key (still validated against HE dir).
    """
    ...
    for he in keys:
        ...
        if require_cells_csv and not cells_with_pixel_path(sample_dir, he).is_file():
            print(f"  skip {he}: missing cells_with_pixel.csv (run match_xenium_cells_with_pixel.py)", flush=True)
            continue
    ...
    if not out:
        raise FileNotFoundError(
            f"No BRCA samples under {cases_root}. "
            "Run match_xenium_cells_with_pixel.py and/or --steps he_h5ad first."
        )
    return out
...
def prepare_pooled_cv_data_from_h5ads(
    ...
    """Pool HE matched h5ads for BRCA three-head (L2/L12/L1)."""
...
def resolve_data_root(args: argparse.Namespace, cases_root: Path) -> Path:
    """Pooled result parent (``data/Xemium/BRCA/Results``). Override with --data-root."""
    if getattr(args, "data_root", None) is not None:
        return args.data_root.expanduser().resolve()
    if cases_root.resolve() == DEFAULT_CASES_ROOT.resolve():
...
def step_he_h5ad(ctx: RunContext) -> None:
    ...
    if not cell_coords.is_file():
        raise FileNotFoundError(
            ...
            "Run: conda run -n SeededNTM python code/Xenium_brca/match_xenium_cells_with_pixel.py "
...
def step_prepare_cv(ctx: RunContext) -> None:
    ...
    if use_three_head:
        print(
            "  Three-head training (BRCA): L2=final_CT, L12=final_sublineage, "
            "L1=final_lineage",
            flush=True,
        )
...
def step_pooled_prepare(ctx: PooledRunContext) -> None:
    ...
    if use_three_head:
        print(
            "  Three-head pooled training (BRCA): L2=final_CT, L12=final_sublineage, "
            "L1=final_lineage",
            flush=True,
        )
    ctx.g.update(
...
def step_pooled_train(ctx: PooledRunContext) -> None:
    ...
    n_groups = len(ctx.samples)
    n_splits = min(int(ctx.cv_k), max(n_groups, 1))
    if n_splits != ctx.cv_k:
        print(
            f"  reducing cv_k {ctx.cv_k} → {n_splits} (n_samples={n_groups})",
            flush=True,
        )

    print(
        f"\n[Pooled train] group CV k={n_splits} train_frac={ctx.train_group_frac} "
...
POOLED_STARDIST_LEVELS: tuple[str, ...] = ("l2", "l1", "l12")  # BRCA three-head
...
########################################################
## StarDist all-nuclei inference for extra StarDist-only regions (none for BRCA preview)
########################################################
def discover_brca_rest_samples(
    ...
    *,
    ...
    """BRCA preview has only annotated rep1/rep2; no unlabeled rest regions."""
    return []
...
def discover_hcc_rest_samples(
    ...
    *,
    ...
    """Compatibility alias used by the HCC-style all notebook."""
    return discover_brca_rest_samples(
        cases_root, stardist_root, require_h5ad=require_h5ad
    )
...
def step_pooled_stardist_all_hcc_rest(
    ...
    *,
    ...
    """
    Predict all StarDist nuclei for extra regions without cell-type CSV.

    BRCA preview has none; kept so the HCC-style all notebook can call it.
    Output: ``Results/{save_result}/stardist_brca_rest/{sample}/..._label.h5ad``
    """
    cases_root = Path(cases_root or ctx.cases_root)
    if samples is None:
        samples = discover_brca_rest_samples(
            cases_root, stardist_root, require_h5ad=True,
        )
    if not samples:
        print(
            f"\n[Pooled StarDist all nuclei — BRCA rest] no extra samples "
    ...
    print(
        f"\n[Pooled StarDist all nuclei — BRCA rest] {len(samples)} datasets",
        flush=True,
    )
    failures = []
    for sample in samples:
        try:
            out_path = stardist_brca_rest_all_label_h5ad_path(
                ctx.data_root, sample, ctx.save_result
            )
    ...
    if failures:
        print(
            f"\n  StarDist-all BRCA rest failures: {len(failures)}/{len(samples)}",
            flush=True,
        )
...
def step_pooled_stardist_all_incomplete(
    ...
    *,
    ...
    """
    Predict all StarDist nuclei for Incomplete_Cases using the pooled checkpoint.

    Input: ``Incomplete_Cases/{sample}/{sample}_all_features_stardist.h5ad``
    Output: ``Data/{save_result}/stardist_Incomplete_Cases/{sample}/..._label.h5ad``
    """
    cases_root = cases_root or (ctx.data_root / INCOMPLETE_CASES_DIRNAME)
    if samples is None:
        samples = discover_stardist_all_h5ad_samples(cases_root)
    if not samples:
        print(f"\n[Pooled StarDist all nuclei — Incomplete_Cases] no samples under {cases_root}", flush=True)
        return
    ...
    for sample in samples:
        try:
            ...
            failures.append((sample, str(exc)))
    if failures:
        print(f"\n  StarDist-all Incomplete failures: {len(failures)}/{len(samples)}", flush=True)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Xenium BRCA UNI-label train/validate (rep1 and rep2; HE + StarDist).",
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help=f"Cases root with rep1/rep2 dirs (default: {DEFAULT_CASES_ROOT}).",
    )
    p.add_argument(
        "--data-root",
        type=Path,
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Single MATCHED_HE key (default: rep1 and rep2).",
    )
    p.add_argument(
        "--therapy-model",
        type=str,
    ...
    p.add_argument(
        ...
        metavar="DIR",
        help=(
            f"Cross-dataset output subfolder under Results/ (default: {DEFAULT_POOLED_SAVE_RESULT!r}). "
            "Example: --pooled-save-result result_all_spatial → Results/result_all_spatial/."
        ),
    )
    p.add_argument(
        "--mode",
        ...
        default="per-sample",
        help="per-sample: train/validate each replicate separately; "
        "cross-dataset: pool BRCA h5ads, dataset-level CV, "
        f"outputs under Results/{{pooled_save_result}} (default {DEFAULT_POOLED_SAVE_RESULT!r}).",
    )
    p.add_argument(
        "--train-group-frac",
        type=float,
        ...
        "(ceil(n×frac) train; default 0.7 → 2 samples still split 1 train / 1 test).",
    )
    p.add_argument(
        "--steps",
        nargs="+",
    ...
    p.add_argument(
        ...
        help=(
            ...
            "three_tier_auc_sum = sum of L2+L1+L12 macro AUROC (BRCA default)."
        ),
    )
    p.add_argument(
        "--cv-selection-metric",
...
def resolve_save_result(
    ...
    """
    Resolve output subfolder name under Results/ (cross-dataset) or per-sample project dir.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.mode == "cross-dataset":
        ...
        for sample in samples:
            if sample not in acq_map:
                raise KeyError(f"Unknown BRCA sample {sample!r}")
        ...
        samples = discover_brca_samples(cases_root, args.sample, require_h5ad=True)

        print(f"Mode:       cross-dataset (pooled BRCA)", flush=True)
    ...
    samples = discover_brca_samples(cases_root, args.sample, require_h5ad=False)
    column_rename = dict(HCC_COLUMN_RENAME)

    print(f"Cases root: {cases_root}", flush=True)
    print(f"Samples:    {len(samples)} (Xenium BRCA rep1/rep2)", flush=True)
    print(f"Steps:      {', '.join(sorted(steps))}", flush=True)
    print(f"Device:     {device}", flush=True)
    print(f"Train batch size: {args.train_batch_size}", flush=True)
    ...
    for sample in samples:
        if sample not in acq_map:
            failures.append((sample, f"Unknown BRCA sample {sample!r}"))
            skipped += 1
            continue
        ctx = RunContext(
            sample=sample,
