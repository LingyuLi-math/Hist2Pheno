...
#!/usr/bin/env python3
"""
Build matched AnnData files for CODEX HCC HE regions.
...
DEFAULT_CASES_ROOT = (
    _REPO_ROOT / "data/CODEX/HCC/Michael_data_transfer/s4769/HE"
)
DEFAULT_STARDIST_ROOT = _REPO_ROOT / "data/CODEX/HCC/StarDist_Segment"
DEFAULT_THERAPY_MODEL = "project_all_UNI"
MAPPING_CSV_NAME = "s4769_he_mapping_updated.csv"
...
def load_acquisition_map(cases_root: Path) -> dict[str, str]:
    ...
    required = {"MATCHED_HE", "CODEX_ACQUISITION_ID"}
    ...
    for _, row in mapping.iterrows():
        ...
        if he_key and he_key.lower() != "nan" and acq_id and acq_id.lower() != "nan":
            out[he_key] = acq_id
    return out
...
def discover_samples(
    ...
    if not samples:
        raise FileNotFoundError(
            f"No mapped HCC sample directories found under {cases_root}"
        )
    return samples
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build CODEX HCC matched and all-nuclei UNI h5ad files.",
    ...
    parser.add_argument(
        ...
        help="Root containing HE/{HE_KEY} directories (repository default).",
    )
    parser.add_argument(
        "--stardist-root",
        type=Path,
        default=DEFAULT_STARDIST_ROOT,
        help="Root containing StarDist_Segment/{HE_KEY} directories.",
    )
    parser.add_argument(
        "--sample",
        default=None,
        help="One MATCHED_HE key; default processes all mapped on-disk HCC regions.",
    )
    parser.add_argument(
        "--steps",
        nargs="+",
