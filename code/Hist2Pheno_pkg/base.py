...
# 2026.08.11 LLY, add function for CODEX_HCC dataset
######################################
class ThreeHeadImprovedMLPClassifier(DualHeadImprovedMLPClassifier):
    """L2 fine + L1 coarse + L12 intermediate heads (no niche heads)."""
    ...
    def __init__(
        ...
        self.level12_head = nn.Linear(hidden_dims[-1], num_level12_classes)
        nn.init.kaiming_normal_(
            self.level12_head.weight, mode="fan_out", nonlinearity="relu"
        )
        if self.level12_head.bias is not None:
            nn.init.constant_(self.level12_head.bias, 0)

    def forward_heads(self, x, neighbor_x=None):
        x = self._apply_spatial(x, neighbor_x)
        feat = self._forward_backbone(x)
        return self.output_layer(feat), self.level1_head(feat), self.level12_head(feat)

    def forward(self, x, neighbor_x=None):
        logits_l2, _, _ = self.forward_heads(x, neighbor_x=neighbor_x)
        return logits_l2
######################################
...
######################################################
# 2026.08.11 LLY, update obs_columns parameter for CODEX_HCC dataset
######################################################
def _matched_h5ad_usable(h5ad_path, obs_columns=XENIUM_H5AD_OBS_COLUMNS):
    ...
    try:
        ...
        ok = all(c in adata.obs.columns for c in obs_columns)    
...
def match_hist2cell_h5ad(
    ...
    if not force_rebuild and _matched_h5ad_usable(
        matched_h5ad_path, obs_columns=obs_columns
    ):
...
def prepare_data_from_matched_h5ad(
    ...
    *,
    ...
    required_cols = [y_col, y_level1_col, y_level12_col]
    if require_niche_heads:
        required_cols.extend([y_level3_col, y_level4_col])
    ...
    y_level3 = (
        adata.obs[y_level3_col].to_numpy() if y_level3_col in adata.obs.columns else None
    )
    y_level4 = (
        adata.obs[y_level4_col].to_numpy() if y_level4_col in adata.obs.columns else None
    )
...
# 2026.08.11 LLY, add function for CODEX_HCC dataset
######################################
def result_has_level12_head_labels(result):
    """True when train/val splits include the intermediate L12 target."""
    return (
        "y_train_level12_encoded" in result
        and "y_test_level12_encoded" in result
    )
######################################
...
def loader_train_test(
    ...
    use_level12 = result_has_level12_head_labels(result)
    ...
    ######################################################
    # 2026.08.11 LLY, add condition for CODEX_HCC dataset
    elif use_level12:
        y_train_l12_tensor = torch.as_tensor(
            result["y_train_level12_encoded"], dtype=torch.long
        )
        y_test_l12_tensor = torch.as_tensor(
            result["y_test_level12_encoded"], dtype=torch.long
        )
        if use_spatial:
            train_dataset = TensorDataset(
                X_train_tensor, y_train_l2_tensor, y_train_l1_tensor,
                y_train_l12_tensor, train_gidx,
            )
            val_dataset = TensorDataset(
                X_test_tensor, y_test_l2_tensor, y_test_l1_tensor,
                y_test_l12_tensor, val_gidx,
            )
        else:
            train_dataset = TensorDataset(
                X_train_tensor, y_train_l2_tensor, y_train_l1_tensor,
