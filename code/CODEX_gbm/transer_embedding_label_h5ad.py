...
#!/usr/bin/env python3
"""
Build matched AnnData files for CODEX GBM HE regions.
...
MAPPING_CSV_NAME = None  # GBM samples are P174511_Initial/P179161_Recurrent
...
## 2026.09.03 GBM: sample id is the UNI embedding prefix (sc_P174511_Initial / sc_P179161_Recurrent)
def load_acquisition_map(
    cases_root: Path, *, aligned_only: bool = True
) -> dict[str, str]:
    """P174511_Initial/P179161_Recurrent map to themselves (no CODEX acquisition table)."""
    from match_codex_cells_with_pixel import list_aligned_annotated_regions
...
def discover_samples(
    ...
    """Return mapped GBM regions with directories under cases_root."""
    if requested_sample is not None:
        ...
        if requested_sample not in acq_map:
            raise KeyError(
                f"Unknown GBM sample {requested_sample!r}; choose P174511_Initial or P179161_Recurrent"
            )
    ...
    if not samples:
        raise FileNotFoundError(
            f"No mapped GBM sample directories found under {cases_root}"
        )
    return samples
...
def _build_matched_h5ad(
    *,
    ...
    """Build a GBM labeled h5ad with stricter cache metadata than base.py."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build CODEX GBM matched and all-nuclei UNI h5ad files.",
    ...
    parser.add_argument(
        "--sample",
        default=None,
        help="One sample id (P174511_Initial or P179161_Recurrent); default processes both on-disk GBM regions.",
    )
    parser.add_argument(
        "--steps",
        nargs="+",
