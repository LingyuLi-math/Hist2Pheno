...
## GIST s1167: sample key is ACQUISITION_ID (no MATCHED_HE).
## All 550 GIST TMA cores are annotated: he_h5ad / stardist_csv / stardist_h5ad / stardist_all_h5ad
...
#!/usr/bin/env python3
"""
Build matched AnnData files for CODEX GIST (s1167 GIST TMA) cores.
...
All 550 GIST cores have cell-type CSV; there is no Incomplete_Cases track.
"""
...
DEFAULT_STARDIST_ROOT = (
    _REPO_ROOT / "data/CODEX/HCC/StarDist_Segment_gist/gist_result"
)
...
def load_acquisition_map(
    ...
    """ACQUISITION_ID → ACQUISITION_ID for the 550 annotated GIST TMA cores."""
    del cases_root  # sample dirs are resolved relative to s1167 via metadata
    region_df = list_aligned_annotated_regions()
    out: dict[str, str] = {}
...
def discover_samples(
    ...
    """Return GIST cores with directories under cases_root."""
    if requested_sample is not None:
        if requested_sample not in acq_map:
            raise KeyError(
                f"Unknown GIST ACQUISITION_ID={requested_sample!r}"
            )
    ...
    if not samples:
        raise FileNotFoundError(
            f"No annotated GIST directories found under {cases_root}"
        )
    return samples
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build CODEX GIST matched and all-nuclei UNI h5ad files.",
    ...
    parser.add_argument(
        ...
        help="StarDist_Segment_gist/gist_result root.",
    )
    parser.add_argument(
        "--sample",
        default=None,
        help="One ACQUISITION_ID; default is all 550 annotated GIST cores.",
    )
    parser.add_argument(
        "--steps",
        nargs="+",
