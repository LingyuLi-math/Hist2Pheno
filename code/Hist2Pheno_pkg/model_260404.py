...
def train_and_save_model_from_split(
    ...
    if skip_if_checkpoint_exists and os.path.exists(save_bestmodel_path):
        print(
            f"Checkpoint already exists, skipping training: {save_bestmodel_path}"
        )
        input_dim, num_classes = infer_input_dim_and_num_classes(
        ...
        _, macro_f1, weighted_f1, _, _ = evaluate(model, eval_loader, device)
        print(
            "Loaded existing checkpoint metrics: "
            f"weighted_f1={weighted_f1:.4f}, macro_f1={macro_f1:.4f}"
        )
...
######################################################
import torch
def load_model_for_predict(path, therapy_data, therapy_model, model_class=ImprovedMLPClassifier, device=None):
    """
    ...
    Returns:
        model_star (nn.Module): Loaded StarDist model.
        checkpoint_path (str): Path to the loaded checkpoint.
    """
    ...
    # Infer input and output dimensions from checkpoint
    input_dim = state_dict["input_layer.0.weight"].shape[1]
