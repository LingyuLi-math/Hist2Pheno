...
# 2026.08.20 LLY
####################################
## Analog of code/CODEX_hcc/match_codex_cells_with_pixel.py for CODEX GIST (s1167 GIST TMA).
##
...
##
## Default: all 550 GIST TMA cores (c009 160 + c011 197 + c013 193).
## Every GIST core has a cell-type CSV (no Incomplete_Cases).
##
...
#
# conda run -n SeededNTM python code/CODEX_gist/match_codex_cells_with_pixel.py --no-stardist

"""Build CODEX GIST GT / StarDist cell tables with HE pixel coordinates."""
...
DEFAULT_STARDIST_ROOT = DEFAULT_STARDIST_GIST_RESULT_DIR
...
HCC_COLUMN_RENAME = GIST_COLUMN_RENAME
...
def sample_he_dir(
    ...
    *,
    ...
    """GIST analog of HCC ``HE/{MATCHED_HE}/``: the acquisition folder itself."""
...
def stardist_csv_path(stardist_root: Path | str, acq_id: str) -> Path:
    """Prepared StarDist table under StarDist_Segment_gist/gist_result/{acq}/."""
    ...
    *,
    ...
    if isinstance(cells_with_pixel, (str, Path)):
        celltype_codex = load_cell_pixcoords(
            ...
            column_rename=GIST_COLUMN_RENAME,
            auto_rename=False,
        )
    else:
        tmp = cells_with_pixel.copy()
        celltype_codex = tmp.rename(columns=GIST_COLUMN_RENAME)
    ...
    restore = {
        tgt: src
        for src, tgt in GIST_COLUMN_RENAME.items()
...
def list_aligned_annotated_regions(
    *,
    ...
    """GIST TMA cores that have on-disk CODEX cell-type CSVs (550; all annotated).
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Prebuild CODEX GIST *_cells_with_pixel.csv and "
            "*_cells_matched_by_stardist.csv for the 550 annotated cores."
        )
    )
    parser.add_argument(
        "--base-dir",
    ...
    parser.add_argument(
        ...
        help="Prepared StarDist gist_result root.",
    )
    parser.add_argument(
        "--acq-id",
        action="append",
        default=None,
        help="ACQUISITION_ID(s); default = all 550 annotated GIST cores.",
    )
    parser.add_argument(
        "--he-key",
        action="append",
    ...
    hierarchy_df = load_s1167_celltype_hierarchy(base_dir=base_dir, cohort="GIST TMA")
