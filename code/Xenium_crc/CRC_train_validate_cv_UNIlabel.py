## 2026.09.15 LLY: Xenium CRC CLI twin of Xenium_brca / CODEX_hcc UNI-label train
## Paths: data/Xemium/CRC/Cases/{P1CRC,P2CRC,P5CRC}/project_all_UNI/
## Samples: P1CRC / P2CRC / P5CRC (Oliveira et al.; transferred Visium HD RCTD labels)
## Hierarchy: three-head (L2/L12/L1); selection metric three_tier_auc_sum
## Spatial: build_spatial_neighbor_index defaults k=8, mode=mean; enabled by default

## cd /home/lingyu/ssd2/Python/Hist2Pheno
## conda activate SeededNTM

## Preprocess CSVs (all samples):
...
# python -u code/Xenium_crc/CRC_train_validate_cv_UNIlabel.py

## Cross-dataset pool (P1+P2+P5) + spatial kNN:
# python -u code/Xenium_crc/CRC_train_validate_cv_UNIlabel.py \
#   --mode cross-dataset \
...
# python -u code/Xenium_crc/transer_embedding_label_h5ad.py --steps stardist_all_h5ad


#!/usr/bin/env python3
"""
Xenium CRC: stratified K-fold HCE training + HE internal validation + StarDist external validation.

Mirrors ``CRC_train_validate_cv_UNIlabel_single.ipynb`` and the HCC three-head CLI.
...
centroids from ``data/Xemium/CRC/StarDist_Segment/``.

By default processes every CRC sample with a Cases CSV (P1CRC / P2CRC / P5CRC)
and enables spatial neighbor fusion with ``build_spatial_neighbor_index``
(k=8, mode=mean).
...
``data/Xemium/CRC/Results/{save_result}/`` (cross-dataset).
"""
...
_CRC_DIR = _REPO_ROOT / "code" / "Xenium_crc"
for _p in (_PKG_DIR, _CRC_DIR):
...
DEFAULT_DATA_ROOT = DEFAULT_CRC_ROOT / "Results"
DEFAULT_STARDIST_ROOT = DEFAULT_CRC_ROOT / "StarDist_Segment"

STARDIST_ALL_H5AD_SUFFIX = "_all_features_stardist.h5ad"
STARDIST_CRC_REST_RESULT_SUBDIR = "stardist_crc_rest"
INCOMPLETE_CASES_DIRNAME = "Incomplete_Cases"


def stardist_crc_rest_all_label_h5ad_path(data_root, sample, save_result="result"):
    """``Results/{save_result}/stardist_crc_rest/{sample}/..._label.h5ad``."""
    result_dir = Path(data_root) / save_result / STARDIST_CRC_REST_RESULT_SUBDIR / sample
...
def discover_crc_samples(
    ...
    *,
    ...
    """
    Return MATCHED_HE keys for Xenium CRC (P1CRC / P2CRC / P5CRC).
    ...
    if not out:
        raise FileNotFoundError(
            f"No CRC samples under {cases_root}. "
...
def prepare_pooled_cv_data_from_h5ads(
    ...
    """Pool HE matched h5ads for CRC three-head (L2/L12/L1)."""
...
def resolve_data_root(args: argparse.Namespace, cases_root: Path) -> Path:
    """Pooled result parent (``data/Xemium/CRC/Results``). Override with --data-root."""
...
def step_prepare_cv(ctx: RunContext) -> None:
    ...
    if use_three_head:
        print(
            "  Three-head training (CRC): L2=final_CT, L12=final_sublineage, "
            "L1=final_lineage",
            flush=True,
        )
...
def step_pooled_prepare(ctx: PooledRunContext) -> None:
    ...
    if use_three_head:
        print(
            "  Three-head pooled training (CRC): L2=final_CT, L12=final_sublineage, "
            "L1=final_lineage",
            flush=True,
        )
    ctx.g.update(
...
POOLED_STARDIST_LEVELS: tuple[str, ...] = ("l2", "l1", "l12")  # CRC three-head
...
########################################################
## StarDist all-nuclei inference for extra StarDist-only regions (none for CRC preview)
########################################################
def discover_crc_rest_samples(
    cases_root: Path | None = None,
    stardist_root: Path | None = None,
    *,
    require_h5ad: bool = False,
) -> list[str]:
    """P1/P2/P5 CRC are the annotated samples; no unlabeled rest regions."""
    return []
...
def discover_hcc_rest_samples(
    ...
    *,
    ...
    return discover_crc_rest_samples(
...
def step_pooled_stardist_all_hcc_rest(
    ...
    *,
    ...
    CRC preview has none; kept so the HCC-style all notebook can call it.
    Output: ``Results/{save_result}/stardist_crc_rest/{sample}/..._label.h5ad``
    """
    cases_root = Path(cases_root or ctx.cases_root)
    if samples is None:
        samples = discover_crc_rest_samples(
            cases_root, stardist_root, require_h5ad=True,
        )
    if not samples:
        print(
            f"\n[Pooled StarDist all nuclei — CRC rest] no extra samples "
    ...
    print(
        f"\n[Pooled StarDist all nuclei — CRC rest] {len(samples)} datasets",
        flush=True,
    )
    failures = []
    for sample in samples:
        try:
            out_path = stardist_crc_rest_all_label_h5ad_path(
                ctx.data_root, sample, ctx.save_result
            )
    ...
    if failures:
        print(
            f"\n  StarDist-all CRC rest failures: {len(failures)}/{len(samples)}",
            flush=True,
        )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Xenium CRC UNI-label train/validate (P1/P2/P5; HE + StarDist).",
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help=f"Cases root with P1CRC/P2CRC/P5CRC dirs (default: {DEFAULT_CASES_ROOT}).",
    )
    p.add_argument(
        "--data-root",
        type=Path,
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Single MATCHED_HE key (default: all CRC samples with Cases CSVs).",
    )
    p.add_argument(
        "--therapy-model",
        type=str,
    ...
    p.add_argument(
        ...
        "cross-dataset: pool CRC h5ads, dataset-level CV, "
    ...
    p.add_argument(
        ...
        help=(
            ...
            "three_tier_auc_sum = sum of L2+L1+L12 macro AUROC (CRC default)."
        ),
    )
    p.add_argument(
        "--cv-selection-metric",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.mode == "cross-dataset":
        ...
        for sample in samples:
            if sample not in acq_map:
                raise KeyError(f"Unknown CRC sample {sample!r}")
        ...
        samples = discover_crc_samples(cases_root, args.sample, require_h5ad=True)

        print(f"Mode:       cross-dataset (pooled CRC)", flush=True)
    ...
    samples = discover_crc_samples(cases_root, args.sample, require_h5ad=False)
    column_rename = dict(HCC_COLUMN_RENAME)

    print(f"Cases root: {cases_root}", flush=True)
    print(f"Samples:    {len(samples)} (Xenium CRC P1/P2/P5)", flush=True)
    print(f"Steps:      {', '.join(sorted(steps))}", flush=True)
    print(f"Device:     {device}", flush=True)
    print(f"Train batch size: {args.train_batch_size}", flush=True)
    ...
    for sample in samples:
        if sample not in acq_map:
            failures.append((sample, f"Unknown CRC sample {sample!r}"))
            skipped += 1
            continue
        ctx = RunContext(
            sample=sample,
