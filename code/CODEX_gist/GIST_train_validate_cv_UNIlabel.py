## 2026.08.20 LLY: CODEX GIST CLI twin of CODEX_hcc/HCC_train_validate_cv_UNIlabel.py
## Paths: data/CODEX/HCC/Michael_data_transfer/s1167/{ACQUISITION_ID}/project_all_UNI/
## Samples: 550 GIST TMA cores, all with cell-type CSV (list_aligned_annotated_regions)
...
## All 550 annotated GIST cores (per-sample):
# python -u code/CODEX_gist/GIST_train_validate_cv_UNIlabel.py

## Cross-dataset pool of 550 h5ads + spatial kNN:
# python -u code/CODEX_gist/GIST_train_validate_cv_UNIlabel.py \
#   --mode cross-dataset \
...
#   --sample Charvill-94_c013_v001_r001_reg002 --steps stardist_all_h5ad

#!/usr/bin/env python3
"""
CODEX GIST: stratified K-fold HCE training + HE internal validation + StarDist external validation.
...
``data/CODEX/HCC/StarDist_Segment_gist/gist_result/{ACQUISITION_ID}/``.

By default processes all **550** annotated GIST TMA cores and enables spatial
...
DEFAULT_STARDIST_ROOT = _REPO_ROOT / "data/CODEX/HCC/StarDist_Segment_gist/gist_result"

## 2026.08.20, StarDist-all for 550 annotated GIST cores
...
def load_he_acq_map(cases_root: Path | None = None) -> dict[str, str]:
    """ACQUISITION_ID → ACQUISITION_ID (GIST has no separate MATCHED_HE)."""
    from s1167_img_cell_mapping import load_s1167_metadata

    del cases_root
    df = load_s1167_metadata(cohort="GIST TMA", annotated_only=False, with_he_only=True)
    return {str(aid): str(aid) for aid in df["ACQUISITION_ID"].astype(str).str.strip()}


def discover_gist_samples(
    cases_root: Path,
    ...
    *,
    ...
    """
    Return ACQUISITION_ID keys for the 550 annotated GIST cores.
    ...
    if not out:
        raise FileNotFoundError(
            f"No annotated GIST samples under {cases_root}. "
...
def prepare_pooled_cv_data_from_h5ads(
    ...
    """Pool HE matched h5ads for GIST three-head (no CNiche/TNiche)."""
...
def step_he_h5ad(ctx: RunContext) -> None:
    ...
    if not cell_coords.is_file():
        raise FileNotFoundError(
            ...
            "Run: conda run -n SeededNTM python code/CODEX_gist/match_codex_cells_with_pixel.py "
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="CODEX GIST UNI-label train/validate (550 annotated cores; HE + StarDist).",
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Single ACQUISITION_ID (default: all 550 annotated GIST cores).",
    )
    p.add_argument(
        "--therapy-model",
        type=str,
    ...
    p.add_argument(
        ...
        "cross-dataset: pool GIST h5ads, dataset-level CV, "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.mode == "cross-dataset":
        ...
        for sample in samples:
            if sample not in acq_map:
                raise KeyError(f"Unknown GIST ACQUISITION_ID={sample!r}")
        ...
        samples = discover_gist_samples(cases_root, args.sample, require_h5ad=True)
    ...
    samples = discover_gist_samples(cases_root, args.sample, require_h5ad=False)
    column_rename = dict(HCC_COLUMN_RENAME)
    ...
    for sample in samples:
        if sample not in acq_map:
            failures.append((sample, f"Unknown GIST ACQUISITION_ID={sample!r}"))
            skipped += 1
            continue
        ctx = RunContext(
            sample=sample,
