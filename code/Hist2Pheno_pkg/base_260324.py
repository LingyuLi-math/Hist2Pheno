...
def _load_pth_embedding_file(pth_path):
    """Load one .pth file and return a 1D numpy embedding."""
...
def _pth_coord_regex(pth_prefix: str):
    """Regex for filenames like ``{pth_prefix}_{x}_{y}.pth`` (x,y are decimal strings)."""
...
def _grid_coords_within_tolerance(cx, cy, tolerance):
    """
    Candidate (x,y) keys to probe for ``{prefix}_x_y.pth``.

    Includes integer grid points within Euclidean distance <= tolerance, plus the
    exact cell coordinate and a few decimal roundings (some pipelines save
    non-integer names).
    """
    tol = float(tolerance)
...
def _candidate_pth_basenames(pth_prefix: str, ex: float, ey: float):
    """Possible on-disk basenames for a candidate embedding coordinate (ex, ey)."""
    basenames = []
    seen = set()
    ...
    add(f"{pth_prefix}_{ex}_{ey}.pth")
...
def _unique_patch_lattice_indices(xv, yv, tolerance):
    """
    From cell coordinates, build unique integer patch indices to probe on disk.

    1) Round each cell to nearest integer patch index (rint).
    2) Dilate with neighbors within Euclidean distance <= tolerance (same idea
       as per-cell ``_grid_coords_within_tolerance``, but vectorized on *unique*
       rounded centers — avoids O(n_cells) Python nested loops).
    """
    tol = float(tolerance)
    ic = np.stack(
...
def _collect_selective_pth_paths(embedding_dir, patch_ij, pth_prefix, pattern):
    """Map unique patch indices to existing file paths (no torch.load)."""
    seen = set()
    paths = []
    embedding_dir = Path(embedding_dir)
    for ix, iy in tqdm(patch_ij, desc="Resolve .pth paths", total=len(patch_ij)):
...
def _load_single_pth_for_dict(path, pattern):
    """Worker: load one path -> (status, key, emb, err_tuple)."""
    p = Path(path)
...
def load_embeddings_selective(
    ...
    """
    Load only the .pth files needed for rows in ``celltype_df``.

    Avoids ``glob('*.pth')`` and loading the entire embedding directory when it
    contains a very large number of files.

    Notes
    -----
    - ``selective_strategy='batch'`` (default): vectorized unique patch indices +
      parallel ``torch.load``. Use for millions of cells.
    - ``selective_strategy='per_cell'``: legacy row-by-row search (very slow
      for huge cell tables).
    - Putting tensors on GPU does **not** speed up disk read/unpickle; use
      ``load_workers`` for parallel I/O + decode instead.
    """
    embedding_dir = Path(embedding_dir)
    pattern = _pth_coord_regex(pth_prefix)
    ...
    patch_ij = _unique_patch_lattice_indices(xv, yv, tolerance)
    print(f"  Unique patch lattice points to probe: {len(patch_ij):,}")

    paths = _collect_selective_pth_paths(embedding_dir, patch_ij, pth_prefix, pattern)
    ...
    def _wrap_load_pth(p):
        return _load_single_pth_for_dict(p, pattern)
...
def _load_embeddings_selective_per_cell(
    ...
    """Original per-cell selective loader (slow for very large cell counts)."""
    embedding_dir = Path(embedding_dir)
...
def load_embeddings(pth_files, pth_prefix="sc_NCRT"):
    """Load every listed .pth file (full scan mode)."""
    embeddings_dict = {}
    failed_files = []
    pattern = _pth_coord_regex(pth_prefix)
    ...
    for pth_file in tqdm(pth_files, desc="Loading .pth files"):
        m = pattern.match(pth_file.name)
        if not m:
            continue
        x_coord = float(m.group(1))
        y_coord = float(m.group(2))
...
def evaluate_and_plot_on_all_data(
    ...
    if has_labels:
        ...
        all_preds = np.concatenate(all_preds)
        all_labels = None  # No labels for evaluation
        all_acc = None
        all_macro_f1 = None
...
def match_celltype2stardist(celltype_pixel_coords,
    ...
    # Fill in matched coordinates and distances
...
def load_hist_embeddings(
    ...
    If ``celltype_df`` is provided (recommended for huge embedding folders),
    only the .pth files needed for those coordinates are loaded (no directory glob).

    If ``celltype_df`` is None or ``force_full_scan=True``, falls back to
    ``glob('*.pth')`` + full load (only feasible for modest file counts).

    ``load_workers``: parallel ``torch.load`` threads in selective ``batch`` mode
    (0 = auto). GPU does not accelerate reading from disk.
    """
    embedding_dir = Path(embedding_dir)

    if celltype_df is not None and not force_full_scan:
        print("Loading HIPT embeddings from .pth files (selective mode)...")
        ...
        embeddings_dict, failed_files = load_embeddings(pth_files, pth_prefix=pth_prefix)
...
def match_hist2cell_matrix(
    ...
    Args:
        ...
        pth_prefix: filename prefix for tiles, e.g. ``sc_NCRT`` for ``sc_NCRT_x_y.pth``.
        force_full_embedding_scan: if True, glob and load all ``*.pth`` (slow for huge dirs).
        embedding_load_workers: parallel loader threads in selective batch mode (0 = auto).
    ...
    ## If cache exists, skip embedding I/O entirely (huge win for million-file folders).
    if os.path.exists(matched_features_path):
        ...
        ## Build matched embeddings and save cache
        X, y, y_level1, X_coords_matched = prepare_matched_embeddings(
            celltype_pixel_NCRT_tumor1,
            embeddings_dict,
            matched_features_path,
