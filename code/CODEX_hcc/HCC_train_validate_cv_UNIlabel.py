## 2026.08.12 LLY: CODEX HCC CLI twin of Xenium_lung/Lung_train_validate_cv_UNIlabel.py
## Paths: data/CODEX/HCC/Michael_data_transfer/s4769/HE/{MATCHED_HE}/project_all_UNI/
## Samples: 36 ALIGNED regions with CODEX celltype annotation (list_aligned_annotated_regions)
...
## Preprocess CSVs (once per MATCHED_HE):
...
## Cross-dataset pool of 36 h5ads + spatial kNN (lung defaults):
# python -u code/CODEX_hcc/HCC_train_validate_cv_UNIlabel.py \
#   --mode cross-dataset \
...
#!/usr/bin/env python3
"""
CODEX HCC: stratified K-fold HCE training + HE internal validation + StarDist external validation.

Mirrors ``HCC_train_validate_cv_UNIlabel_single.ipynb`` and the Xenium lung CLI
``Lung_train_validate_cv_UNIlabel.py``. Assumes UNI embeddings exist under
``HE/{MATCHED_HE}/project_all_UNI/ImgEmbeddings_all/``. GT / StarDist cell CSVs come from
``match_codex_cells_with_pixel.py``; StarDist centroids from
``data/CODEX/HCC/StarDist_Segment/{MATCHED_HE}/``.

By default processes all **36** ALIGNED annotated HCC regions and enables spatial
neighbor fusion with lung defaults (``build_spatial_neighbor_index``, k=8, mode=mean).
...
Terminal usage (from repo root ``Hist2Pheno``):
  ...
  # All 36 annotated ALIGNED samples (per-sample)
  python -u code/CODEX_hcc/HCC_train_validate_cv_UNIlabel.py

  # Cross-dataset: pool h5ads, dataset-level CV, spatial kNN (lung defaults)
  python -u code/CODEX_hcc/HCC_train_validate_cv_UNIlabel.py \\
    --mode cross-dataset --use-spatial-context --spatial-k 8 --spatial-mode mean \\
    --pooled-save-result result_all_spatial

  # Train only
  python -u code/CODEX_hcc/HCC_train_validate_cv_UNIlabel.py --sample awy-98938_aligned_0d535a74 --steps he_h5ad train

  # StarDist validation only
  python -u code/CODEX_hcc/HCC_train_validate_cv_UNIlabel.py --sample awy-98938_aligned_0d535a74 --steps stardist

CSV outputs under ``HE/{MATCHED_HE}/project_all_UNI/result/``:
...
_CODEX_HCC_DIR = _REPO_ROOT / "code" / "CODEX_hcc"
for _p in (_PKG_DIR, _XENIUM_DIR, _CODEX_HCC_DIR):
...
DEFAULT_S4769_ROOT = (
    _REPO_ROOT / "data/CODEX/HCC/Michael_data_transfer/s4769"
)
...
def load_he_acq_map(cases_root: Path | None = None) -> dict[str, str]:
    """MATCHED_HE → CODEX_ACQUISITION_ID from s4769_he_mapping_updated.csv."""
    he_root = Path(cases_root) if cases_root is not None else DEFAULT_CASES_ROOT
    mapping_csv = he_root / "s4769_he_mapping_updated.csv"
    if not mapping_csv.is_file():
        raise FileNotFoundError(mapping_csv)
    df = pd.read_csv(mapping_csv)
    out = {}
    for _, row in df.iterrows():
        he = str(row["MATCHED_HE"])
        out[he] = str(row["CODEX_ACQUISITION_ID"])
    return out


def discover_hcc_samples(
    cases_root: Path,
    ...
    *,
    ...
    """
    Return MATCHED_HE keys for the 36 ALIGNED annotated HCC regions.
    ...
    if not out:
        raise FileNotFoundError(
            f"No HCC samples under {cases_root}. "
...
def prepare_pooled_cv_data_from_h5ads(
    ...
    """Pool HE matched h5ads for HCC three-head (no CNiche/TNiche)."""
...
def resolve_data_root(args: argparse.Namespace, cases_root: Path) -> Path:
    """Pooled result parent (s4769/). Override with --data-root."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="CODEX HCC UNI-label train/validate (36 ALIGNED regions; HE + StarDist).",
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help=f"HE root with MATCHED_HE dirs (default: {DEFAULT_CASES_ROOT}).",
    )
    p.add_argument(
        "--data-root",
        type=Path,
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Single MATCHED_HE key (default: all 36 ALIGNED annotated regions).",
    )
    p.add_argument(
        "--therapy-model",
        type=str,
    ...
    p.add_argument(
        ...
        metavar="DIR",
        help=(
            f"Cross-dataset output subfolder under s4769/ (default: {DEFAULT_POOLED_SAVE_RESULT!r}). "
            "Example: --pooled-save-result result_spatial → s4769/result_spatial/."
        ),
    )
    p.add_argument(
        "--mode",
        ...
        default="per-sample",
        help="per-sample: train/validate each MATCHED_HE separately; "
        "cross-dataset: pool HCC h5ads, dataset-level CV, "
        f"outputs under s4769/{{pooled_save_result}} (default {DEFAULT_POOLED_SAVE_RESULT!r}).",
    )
    p.add_argument(
        "--train-group-frac",
        type=float,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.mode == "cross-dataset":
        ...
        for sample in samples:
            if sample not in acq_map:
                raise KeyError(f"No CODEX_ACQUISITION_ID for MATCHED_HE={sample!r}")
        ...
        samples = discover_hcc_samples(cases_root, args.sample, require_h5ad=True)
    ...
    samples = discover_hcc_samples(cases_root, args.sample, require_h5ad=False)
    column_rename = dict(HCC_COLUMN_RENAME)
    ...
    for sample in samples:
        if sample not in acq_map:
            failures.append((sample, f"No CODEX_ACQUISITION_ID for MATCHED_HE={sample!r}"))
            skipped += 1
            continue
        ctx = RunContext(
            sample=sample,
